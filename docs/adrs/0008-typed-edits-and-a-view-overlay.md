...
- **Limits that a linked axes cannot show.** The axes of a link group may differ in scale, so limits that suit one member can be undrawable on another: a range that reaches zero cannot be shown on a logarithmic axis. A command synchronises the limits only onto the axes of the group that can show them, and such a member keeps its own limits, rather than the whole transaction being refused. Linking therefore never produces a figure that validation rejects, and linking every axes of a figure cannot fail. `set_limits` still sets the axes it was asked about, so a range that that axes cannot show is refused when the transaction is applied.
