...
- **An interactive viewer.** A desktop window shows one or more figures as tabs, in which axes can be panned, zoomed and rotated, and plots can be hidden or shown from the legend. Every interaction sets a property of the figure model, and can be undone, so the view on screen is always a figure that can be saved or exported.
...
- **Guides** explain how to use IronLAB.
    ...
    - [Using the viewer](guides/viewer.md) describes the viewer's tools, gestures and keyboard shortcuts.
