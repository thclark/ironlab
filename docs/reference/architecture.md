...
| `ironlab-viewer` | The interactive viewer: an eframe application with one tab per figure, a toolbar, the canvas that draws a display list as egui meshes and the three-dimensional axes through the viewer's own depth-tested wgpu pipelines, the interaction state machine, the property editor, an offscreen renderer that draws a figure into an image without a window, and the `ironlab-viewer` binary, which opens `.fig` and JSON files. |
...
```text
   ...
   layout · ticks · limits · colormaps · contours · quivers · images · 3D projection · depth sort
     ...
     lyon → egui meshes      same meshes and lists      krilla → PDF page
     and depth-tested lists  → image                    (export; asks the
     (interactive window)    (gallery images, tests,    offscreen renderer
                             the exporter's rasters)    for its rasters)
```
...
- The **canvas** tessellates paths with lyon (splitting dashed strokes into dashes first) and glyph outlines into triangle meshes, which egui draws with its wgpu renderer using four-times multisample anti-aliasing. An image item is drawn as a textured quad through the same transform chain as every path, so that a mirrored or projected image lands where the compiler placed it: its samples are uploaded as a texture, cut into tiles of at most 8192 pixels on a side where the image is larger than one texture may be, and sampled with nearest filtering so that the edges of its pixels stay hard. The canvas keeps the textures of the images on screen in a cache across frames, keyed by the sample buffer and the tile, so that a pan or a zoom re-uploads nothing, and frees the textures that the latest rebuild of its meshes did not use. The leaves of a depth item become a draw list for the viewer's own wgpu pipelines instead: the same lyon triangles, in screen units, with the depth of every vertex, which a paint callback draws inside egui's render pass with a depth test, so that faces, lines, markers and images that cross one another are resolved pixel by pixel. The window is created with a depth attachment for this purpose; the pipelines share egui's mapping of points to pixels and its blending, so that a depth-tested vertex lands on the same pixel as a mesh vertex would. The figure is drawn at its physical aspect ratio, scaled to fit its tab.
- The **offscreen renderer** draws the same meshes and the same draw lists into a texture without a window, through the same pipelines and a depth attachment of its own, and reads the image back, uploading the tiles of each image for the render and freeing them once the image is read back. The documentation gallery's images are made this way, so they show exactly what the viewer shows, and so are the rasters the PDF exporter embeds. Nothing in this path needs a window or a hardware graphics device; see [running without a graphics device](../guides/getting-started.md#running-without-a-graphics-device).
- The **PDF backend** writes each path and glyph run to a single page whose MediaBox and CropBox equal the figure size, with fonts embedded as subsets, and embeds each image item as a deflated image XObject at its own resolution, drawn without interpolation beneath the transform of its group and with a soft mask only when it has alpha. Every item is checked before it reaches krilla and anything that cannot be represented is skipped, so the exporter always writes a valid PDF, including from a display list assembled by hand rather than compiled from a figure. The decisions behind PDF-first export, and behind the raster fallback below, are recorded in [ADR 0010](../adrs/0010-pdf-export-with-a-raster-fallback.md), and those behind the image artists in [ADR 0011](../adrs/0011-image-artists.md).
