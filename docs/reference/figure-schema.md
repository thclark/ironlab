...
- **New axes-level decorations are new axes properties.** A colorbar, for example, becomes an optional property of an axes that shows the colour scale of that axes and holds no colormap or colour limits of its own, so that it cannot disagree with them ([ADR 0011](../adrs/0011-colour-scales-are-referenced-table-entries.md)).
