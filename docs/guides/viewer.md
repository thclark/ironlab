...
A value that holds other values, such as the x axis or the style of a line, is a heading carrying its name alone: a band across the panel, set in the same spaced capitals as the headings of the figure browser, beneath which the values it holds are listed. The heading is never a value in itself, because everything it holds is already on screen below it.
