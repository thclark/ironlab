...
//! axis. Such a member is **skipped** and keeps its own limits, rather than the whole
//! transaction being refused, so that linking never produces a figure that validation
//! rejects. [`set_limits`] always sets the axes it was asked about, so a request that
//! that axes cannot accept is still refused when the transaction is applied.

use crate::axes::{Axes, Limits, Projection, Scale};
...
/// figure, and nothing else. An axes of the group other than the one named that cannot
/// accept the limits (a logarithmic axis and limits that are not positive) is skipped
/// and keeps its own limits; the axes named is always set, so applying the transaction
/// refuses limits that it cannot accept.
///
/// # Errors
///
...
pub fn set_limits(
    ...
    let edits = figure
        ...
        .into_iter()
        .filter(|&id| id == axes || accepts(figure, id, dimension, limits))
...
///
/// An axes of the group that cannot accept the reference limits is skipped and keeps its
/// own limits, so linking never produces a figure that validation rejects.
///
/// # Errors
///
...
pub fn link(
    ...
    edits.extend(
        group
            .into_iter()
            .filter(|&id| id == reference || accepts(figure, id, dimension, limits))
            .map(|id| set_limits_edit(id, dimension, limits)),
    );
    Ok(Transaction { edits })
}
...
/// Returns whether an axes can be given the limits without making the figure invalid.
///
/// Limits that are not finite and strictly increasing are accepted by no axes, and
/// limits that are not positive are accepted by no logarithmic axis. The z axis of a
/// two-dimensional axes accepts every range, because it is ignored when the figure is
/// drawn and when it is validated.
fn accepts(figure: &Figure, axes: NodeId, dimension: Dimension, limits: Limits) -> bool {
    ...
    let three_d = |axes: &Axes| matches!(axes.projection, Projection::ThreeD { .. });
    if dimension == Dimension::Z && !figure.axes(axes).is_some_and(three_d) {
        return true;
    }
    axis_of(figure, axes, dimension).scale != Scale::Log || min > 0.0
}
